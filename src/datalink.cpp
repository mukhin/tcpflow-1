...
void dl_null(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
	...
	DEBUG(6) ("warning: received null frame with unknown type (type %d)", family);
	return;
    }
#endif

    process_ip(&h->ts,p + NULL_HDRLEN, caplen - NULL_HDRLEN,flow_t::NO_VLAN);
}
...
void dl_ethernet(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
    ...
    int32_t vlan = flow_t::NO_VLAN;			       /* default is no vlan */
	...
	process_ip(&h->ts,ether_data, caplen - sizeof(struct ether_header),vlan);
	return;
    }

    /* Unknown Ethernet Frame Type */
    DEBUG(6) ("warning: received ethernet frame with unknown type %x", ntohs(eth_header->ether_type));
}
...
void dl_ppp(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
    ...
    process_ip(&h->ts,p + PPP_HDRLEN, caplen - PPP_HDRLEN,flow_t::NO_VLAN);
}
...
void dl_raw(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
    ...
    process_ip(&h->ts,p, caplen,flow_t::NO_VLAN);
}
...
void dl_linux_sll(u_char *user, const struct pcap_pkthdr *h, const u_char *p){
    ...
    process_ip(&h->ts,p + SLL_HDR_LEN, caplen - SLL_HDR_LEN,flow_t::NO_VLAN);
}
